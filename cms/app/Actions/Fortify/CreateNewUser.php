...
class CreateNewUser implements CreatesNewUsers
{
    ...
    private function ensureRegistrationAllowed(string $ip): void
    {
        ...
        $maxAccountsPerIp = (int) (env('MAX_ACCOUNTS_PER_IP') ?? setting('max_accounts_per_ip') ?? 99);

        if ($maxAccountsPerIp > 0 && $matchingIpCount >= $maxAccountsPerIp) {
            throw ValidationException::withMessages(['registration' => __('You have reached the limit of maximum allowed accounts')]);
        }
    ...
    private function validate(array $inputs): array
    {
        $rules = [
            ...
            'cf-turnstile-response' => ['sometimes', 'nullable', 'string', app(Turnstile::class)],
        ];
